...
module Errors = struct
  type t =
    ...
    (* Evaluator/Runtime *)
    | Invalid_Type of Location.t * string * string
  ...
  let invalid_type loc typename expected =
    raise (Error (Invalid_Type (loc, typename, expected)))
  ...
  let to_string (msg : t) : string =
    ...
    | Invalid_Type (loc, typename, expected) ->
      Printf.sprintf "%s: Invalid type %s expected %s"
      (Location.to_string loc) 
      typename expected
